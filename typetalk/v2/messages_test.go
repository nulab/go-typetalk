...
func Test_MessagesService_GetDirectMessages_errorResponse(t *testing.T) {
	_, _, err := client.Messages.GetDirectMessages(context.Background(), "qwerty", "test", &GetMessagesOptions{
		10,
		1,
		"backword",
	})
...
func Test_MessagesService_PostDirectMessage_errorResponse(t *testing.T) {
	_, _, err := client.Messages.PostDirectMessage(context.Background(), "qwerty", "test", "hello", &PostMessageOptions{
		ReplyTo:      2,
		ShowLinkMeta: true,
		FileKeys:     []string{"key0", "key1", "key2"},
		TalkIds:      []int{0, 1, 2},
...
func Test_MessagesService_SearchMessages_errorResponse(t *testing.T) {
	from := time.Date(2018, time.May, 1, 0, 0, 0, 0, time.Local)
	to := time.Date(2018, time.May, 3, 0, 0, 0, 0, time.Local)
	query := &SearchMessagesOptions{TopicIDs: []int{}, HasAttachments: true, AccountIDs: []int{}, From: &from, To: &to}
	_, _, err := client.Messages.SearchMessages(context.Background(), "qwerty", "hello", query)
